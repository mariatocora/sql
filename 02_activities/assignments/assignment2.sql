...
customer_id,
market_date,
DENSE_RANK() OVER(PARTITION BY customer_id ORDER BY market_date ASC) as [customer_visit_dense_rank]
...
--STEP 1: I created a temporary table
...
--STEP 2: I calculated the best and wors days based on the temporary table created before
SELECT 
market_date
,sales
FROM temp.total_sales
WHERE sales = (SELECT MAX(sales) from temp.total_sales)
UNION ALL
SELECT 
market_date
,sales
FROM temp.total_sales
WHERE sales = (SELECT MIN(sales) from temp.total_sales);
--Comment: I got a table listing the best and the worst sale days. 
...
CROSS JOIN vendor v on v.vendor_id = vi.vendor_id
...
DROP TABLE IF EXISTS product_units --Included this part in the code because I ran the code multiple times.
...
--Step 1:  Make temporary table
DROP TABLE IF EXISTS product_units --Included this part in the code because I ran the code multiple times.
CREATE TEMPORARY TABLE  temp.older_record AS
SELECT product_id
...
--Step 1: Check the product_id table to confirm the new column was created
SELECT *
FROM product_units

--Step 2: Upadte the table
...
SET current_quantity = (
  ...
  FROM (
    ...
    LEFT JOIN ( ---left join was the only one that replace the nulls, I tried other join options. 
      SELECT *
      ,ROW_NUMBER() OVER( PARTITION BY vi.product_id ORDER BY vi.market_date DESC) AS x
      FROM vendor_inventory vi 
